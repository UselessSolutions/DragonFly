...
public class EntityDiggingFXMixin extends EntityFX {
	...
	private int particleFromModel(Block instance, Side side, int meta){
		...
		if (model instanceof BlockModelDragonFly){
			tex = TextureRegistry.getIndexOrDefault(((BlockModelDragonFly) model).getModelFromState(block, (int) x, (int) y, (int) z, true).getTexture("particle"), block.getBlockTextureFromSideAndMetadata(Side.BOTTOM, meta));
		} else {
