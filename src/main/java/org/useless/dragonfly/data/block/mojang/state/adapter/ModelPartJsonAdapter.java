...
import org.useless.dragonfly.data.block.mojang.state.Condition;
		...
		if (json.isJsonObject()){
			...
			if (obj.has("apply")){
				...
				if (obj.getAsJsonObject("apply").isJsonArray()){
					...
					data.apply = new AppliedData[arr.size()];
					for (int i = 0; i < data.apply.length; i++) {
						AppliedData var = BlockstateData.GSON.fromJson(arr.get(i), AppliedData.class);
						accumulated += var.weight;
						var.weightAccum = accumulated;
						data.apply[i] = var;
					}
				} else {
					data.apply = new AppliedData[]{BlockstateData.GSON.fromJson(obj.get("apply"), AppliedData.class)};
				}
			...
			if (data.apply == null){
				if (json.isJsonArray()){
					...
					for (int i = 0; i < data.apply.length; i++) {
						data.apply[i] = BlockstateData.GSON.fromJson(arr.get(i), AppliedData.class);
						weightAccum += data.apply[i].weight;
						data.apply[i].weightAccum = weightAccum;
					}
					data.weightAccum = weightAccum;
				} else {
					data.apply = new AppliedData[]{BlockstateData.GSON.fromJson(json, AppliedData.class)};
				}
			}
		} else {
			if (json.isJsonArray()){
